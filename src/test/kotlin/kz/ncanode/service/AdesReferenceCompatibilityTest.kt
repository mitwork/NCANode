 ...
 *
 * Файлов в репозитории нет и, скорее всего, не будет: NCALayer умеет строить
 * уровни выше T только для боевых ключей (трест-анкоры берутся из `cacerts.jks`
 * с боевыми корнями, тестовый `cacerts_test.jks` в их сборке лежит, но не
 * используется), а боевая подпись — это реальные ИИН и ФИО, которым в публичном
 * репозитории не место. Поэтому каждый тест **пропускается**, если своего файла
 * не нашёл: спека рассчитана на локальный прогон у того, у кого эти подписи есть.
 *
