...
**NCALayer не умеет LT/LTA тестовыми ключами.** Их AdES-код берёт
трест-анкоры только из боевого `cacerts.jks`; тестовый `cacerts_test.jks`
в сборке лежит, но не используется. B и T анкоров не требуют, LT — первый
уровень с `buildChain` → `CHAIN_BUILD_FAILED`. Значит эталоны выше T бывают
только боевым ключом, а им в репозиторий нельзя (реальные ИИН/ФИО): лежат
локально, исключены через `.git/info/exclude`, гоняются
`AdesReferenceCompatibilityTest` под профилем `reference` (боевые адреса).
...
1. Коммитабельные эталоны — генератором на их библиотеке с тестовыми анкорами,
